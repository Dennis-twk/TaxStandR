#' GBIF 数据库植物物种全量检索与清洗 (GBIF Database Species Search for Plants)
#'
#' @description
#' 本函数专为 GBIF (Global Biodiversity Information Facility) 数据库设计，专门用于检索植物数据。
#' 具备自动清理乱码、严格匹配以及人工交互复核功能。
#'
#' 针对 GBIF 数据库返回数据繁杂的特点，本模块内置了以下机制：
#' 1. 全界域检索：去除了动物专用的 `class = "Insecta"` 限制；
#' 2. 智能筛选机制：遇到多个同名匹配时，优先捕获 `status == "ACCEPTED"` 的标准接受名；
#' 3. 词尾等效容错：内置 4 大拉丁形容词变格映射（如 -us/-um 映射 -a，-is 映射 -e）；
#' 4. 预防空值出现报错与植物降维：去除了总科阶元提取，并在零匹配时输出标准空表，保障流水线稳定。
#'
...
#'
search_gbif_Pla <- function(raw_names) {
  ...
  # --- 第一轮：严格匹配（去除了 class = "Insecta" 限制） ---
  pb <- txtProgressBar(min = 0, max = length(search_list), style = 3)
  ids_list <- lapply(seq_along(search_list), function(i) {
    res <- auto_retry({ suppressWarnings(suppressMessages(taxize::get_gbifid(search_list[i], ask = FALSE, messages = FALSE))) })
  ...
  if(nrow(df_pass) > 0) {
    ...
    parsed <- parse_taxonomy_Pla(classf, db_name = "GBIF") # 调用植物版专属解析
    res_parsed <- df_pass %>% left_join(parsed, by = "taxid")
  }

  # 安全补齐必填列机制 (防0行数据报错，移除了 superfamily)
  expected_cols <- c("GBIF.kingdom", "GBIF.phylum", "GBIF.class", "GBIF.order",
                     "GBIF.family", "GBIF.genus", "GBIF.species")
  ...
  res_final <- df_map %>% left_join(res_parsed, by = "preprocessed.name") %>%
    ...
    transmute(input.name, cleaned.name = preprocessed.name, taxid,
              ...
              family = GBIF.family, genus = GBIF.genus, species = GBIF.species, source)
  cat("  => 已通过【GBIF】完成所有分类信息的检索！\n")

  # save_path <- 'output/GBIF_results_Pla.xlsx'
  #
  # tryCatch({
  ...
  #   fallback_path <- sprintf('output/GBIF_results_Pla_backup_%s.xlsx', timestamp)
